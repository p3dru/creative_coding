//idéia: fazer a cor da linha ser alterada com base na distância
...
class Agent{                                              //cria uma classe agente com os valores passados em new Vector(x, y); 
  constructor(x, y){
    ...
    this.vel = new Vector(random.range(-1, 1), random.range(-1, 1));    //dá uma velocidade aleatória a cada bolinha
  ...
  wrap(width, height){
    //if (this.pos.x <= 0 || this.pos.x >= width) this.pos.x *= 1;
    //if (this.pos.y <= 0 || this.pos.y >= height) this.pos.y *= -1;
...
const colors = ['#4000ff', '#00ff80', '#54daa4', '	#40ff00', '#ff4000', '#F55D07', '#4CEBDE', '#EBC707', '#A6EB2D', '#7C2FF7']
  ...
  return ({ context, width, height }) => {                //retorno da função sketch
    ...
    for (let i = 0; i < agents.length; i++){
      ...
      for (let j = i + 1; j < agents.length; j++){
        ...
        /*
        if (150 <= context.lineWidth < 200){
          context.strokeStyle = random.pick(colors);
        }
        if (200 <= context.lineWidth < 250){
          context.strokeStyle = random.pick(colors);
        }
        */
        //context.strokeStyle = 'white';  
    ...
    agents.forEach(agent => {                             //para cada item do array
      ...
      agent.wrap(width, height);                        //executa o método para adicionar colisão e alterar o estado
    });
  };
};
